...
from utils.datasets import LoadImages, load_images
...
def load_my_model(model_path):
    ...
    model.load_state_dict(torch.load(str(BASE_DIR / model_path['weights']), map_location=device)['model'])
...
def load_classes(path):
    ...
    with open(BASE_DIR / path['classes'], 'r') as f:
        names = f.read().split('\n')
...
def load_model_with_signal(model_path: str) -> None:
    ...
    if _model is None:
        ...
        _tra_names = load_classes(path=model_path)
...
def predict(data: bytes, model=None):
    """
    ...
    data_set = load_images(frame=frame)
    # clean grad
    img = data_set[0]
    im0 = data_set[1]
    with torch.no_grad():
            ...
            if det is not None and len(det):
                ...
                for *xyxy, conf, cls in det:
                    if True:  # Add bbox to image
                        ...
                        label = '%s' % (_tra_names[int(cls)])
                        plot_one_box(xyxy, im0, label=label, color=colors[int(cls)])

        # check the data
        cv.imshow('test', cv.resize(im0, (612, 612), ))
        cv.waitKey(0)
        cv.destroyAllWindows()
        return cv.resize(im0, (612, 612), )
